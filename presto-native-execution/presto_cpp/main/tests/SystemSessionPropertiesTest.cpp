...
TEST_F(SystemSessionPropertiesTest, validateSessionProperties) {
  SystemSessionProperties systemSessionProperties;
  static const std::vector<std::string> names = {
      "driver_cpu_time_slice_limit_ms", "spill_compression_codec"};
  std::vector<std::shared_ptr<SessionProperty>> expectedSessionProps = {
  ...
  for (auto i = 0; i < names.size(); i++) {
    auto property = systemSessionProperties.getSessionProperty(names[i]);
    EXPECT_EQ(*expectedSessionProps[i], *property);
  }
}

TEST_F(SystemSessionPropertiesTest, testMaxSpillLevel) {
  SessionPropertyReporter propertyReporterObject;
  const std::string sessionPropertyNames = "native_max_spill_level";
  const std::string expectedJsonString = R"(
      {
        "defaultValue":"1",
...
TEST_F(SystemSessionPropertiesTest, testDriverCpuTimeLimit) {
  SessionPropertyReporter propertyReporterObject;
  const std::string sessionPropertyNames = "driver_cpu_time_slice_limit_ms";
