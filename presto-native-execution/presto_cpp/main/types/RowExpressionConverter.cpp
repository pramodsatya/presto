...
SwitchFormArguments RowExpressionConverter::getSwitchSpecialFormArgs(
  ...
  return getSimpleSwitchFormArgs(switchExpr, inputArgs);
}
...
json RowExpressionConverter::getSpecialForm(
  ...
  if (form == kSwitch) {
    ...
    auto switchResult = getSwitchSpecialFormArgs(switchExpr, input);
...
json RowExpressionConverter::veloxToPrestoRowExpression(
  ...
  if (expr->isConstant()) {
    if (expr->inputs().empty()) {
      ...
      // zero here) during evaluation. So the input expression is returned
