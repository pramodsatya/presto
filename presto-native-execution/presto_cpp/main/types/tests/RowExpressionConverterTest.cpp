...
class RowExpressionConverterTest
 ...
 protected:
  ...
  void testFile(
      ...
      std::vector<json> inputRowExpressions = {}) {
    json::array_t expected = json::parse(
        slurp(facebook::presto::test::utils::getDataPath(testName)));
    auto numExpr = inputs.size();
    for (auto i = 0; i < numExpr; i++) {
      json inputRowExpr =
          inputRowExpressions.empty() ? json() : inputRowExpressions.at(i);
      json result = rowExpressionConverter_->veloxToPrestoRowExpression(
...
TEST_F(RowExpressionConverterTest, special) {
  ...
  json rowExprWithTwoInputs;
  rowExprWithTwoInputs["arguments"] = json::array({json(), json()});
  auto inputRowExpressions =
      std::vector<json>{json(), rowExprWithTwoInputs, rowExprWithTwoInputs};
  testFile(inputs, "CallAndSpecialFormExpressions.json", inputRowExpressions);
}
